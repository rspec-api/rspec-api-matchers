...
describe 'have_attributes matcher' do
  ...
  describe 'expect(response).to have_attributes(...)' do
    ...
    it 'passes when given one existing key in an object as a Symbol' do
      response.body = '{"id":1}'
      expect(response).to have_attributes :id
    end

    it 'passes when given one existing key in an object as a String' do
      response.body = '{"id":1}'
      expect(response).to have_attributes 'id'
    end

    it 'passes when given one existing key in an object as a Hash' do
      response.body = '{"id":1}'
      expect(response).to have_attributes id: {}
    end

    it 'passes when given one existing key in an array' do
      response.body = '[{"id":1}, {"id":2}]'
      expect(response).to have_attributes 'id' => {}
    end

    it 'passes when given an empty array' do
      response.body = '[]'
      expect(response).to have_attributes :id
    end

    it 'passes when given more existing keys in an object as an Array' do
      response.body = '[{"id":1,"name":"one"},{"id":2,"name":"two"}]'
      expect(response).to have_attributes [:id, :name]
    end

    it 'passes when given more existing keys' do
      response.body = '[{"id":1,"name":"one"},{"id":2,"name":"two"}]'
      expect(response).to have_attributes id: {}, name: {}
    end
    ...
    it 'passes when given an existing object key/value pair with symbols' do
      response.body = '{"user":{"id":1}}'
      expect(response).to have_attributes user: {value: {id: 1}}
    end
    ...
    it 'passes when given multiple existing key/value pairs' do
      response.body = '[{"id":1,"name":"three"},{"id":2,"name":"three"}]'
      expect(response).to have_attributes name: {value: 'three'}
    end

    # KEY + PROC VALUE

    it 'passes when the value satisfies the :value Proc' do
      response.body = '{"id":1}'
      expect(response).to have_attributes id: {value: -> v {v.odd?}}
    end

    it 'passes when all the multiple values satisfy the :value Proc' do
      response.body = '[{"id":1},{"id":3}]'
      expect(response).to have_attributes id: {value: -> v {v.odd?}}
    end

    # KEY + VALUE + TYPE

    it 'passes when given an existing key/value pair of a given type' do
      response.body = '{"id":null}'
      expect(response).to have_attributes id: {value: nil, type: :null}
    end

    it 'passes when given multiple existing key/value pairs and types' do
      response.body = '[{"id":1,"name":"three"},{"id":2,"name":"three"}]'
